...
function preparingRequerimentFile (chunk, encoding, opts) {
  ...
  return (cb) => {
    if (preparingRequerimentFile.runned === false) {
      preparingRequerimentFile.runned = true

      const originalFileName = chunk.history[0]
      const newFileOut = path.resolve(path.dirname(originalFileName) + '/' + path.basename(originalFileName, '.js') + '.' + requerimentfilename)
 ...
 * @type {Boolean}
 */
preparingRequerimentFile.runned = false
